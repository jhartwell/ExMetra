defmodule ExMetra.Web do
  ...
  @moduledoc """
  This is the module that interacts with the Metra JSON API. This should never be called directly but instead use the other facilities to make the calls like the ExMetra protocol or ExMetra.Query. It uses HTTPoison as the base in order to make the actual calls. It will use the same functions as HTTPoison. Because of the fact that it uses the @endpoint as the base of the url, it will only be able to be used to call the Metra API. Do not use this if you are looking to make calls to other websites or services. Do note that basic authorization will already be present in the web calls so it is not necessary to add the headers when making a request. Because Basic Authorization is already included it is important to set the necessary values in your config.exs. The following values must be present in your config.exs for any web calls to be successful:
